...
スマホでカラーQRリーダーのカメラを使う場合、ページはHTTPSで開く必要があります。PCのローカルIPを `http://...` で開いた場合はブラウザがカメラAPIをブロックするため、HTTPSの公開URLを使うか、リーダー内の「写真を撮って読む」を使います。HTTPSで開けている場合、リーダーは背面カメラを優先して起動します。
